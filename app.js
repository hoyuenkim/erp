...
app.get('/home', loginRequired, async (req, res)=>{
    try{
        ...
        const tableList = await TableModel.find({$and : [{date : {$ne : ''}}, {date : {$gte :  today, $lte : weeks.future}}]}).sort({date : 1});
        const alertList = await TableModel.find({$and : [{date : {$ne : ''}}, {date : {$lt :  today}}]});
        const list = [];
        const serverTable = await TableModel.find({$and : [{date : {$ne : ''}}, {date : {$lte : weeks.future}}]});
        const onepassTable = await TableModel.find({$and : [{date : {$ne : ''}}, {date : {$lte : weeks.future}}]});
        ...
        res.render('home', {orderList : orderList, salesList : salesList, tableList : tableList, alertList : alertList, severStorages : severStorages, onepassStorages : onepassStorages});
        } catch(err){
...
app.get('/update/table', function(req, res){
    const query = {
        ...
        hn : {
            ...
            files : [
                {no : 1, name :'프로토콜', received : false},
                {no : 2, name :'홈넷 IP', received : false},        
            ]
        },
        op : {
            use : false,
...
function getWeeks(){
    ...
    return {today : moment(today).tz("Asia/Seoul").format('YYYY-MM-DD'), future : moment(future).tz("Asia/Seoul").format('YYYY-MM-DD')};
  }
